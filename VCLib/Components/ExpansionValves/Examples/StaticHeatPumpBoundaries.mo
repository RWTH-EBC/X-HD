within VCLib.Components.ExpansionValves.Examples;
...
protected
  model StaticEvaporator
    ...
    AixLib.Fluid.Sources.Boundary_ph source(
      ...
      annotation (
        Placement(transformation(
          ...
          rotation=0,
          origin={0,0})));
    Components.Interfaces.PortsAThroughPortB outVal(redeclare package Medium =
          Medium, nVal=nPorts)
    ...
    AixLib.Fluid.Sensors.SpecificEnthalpyTwoPort senSpeEnt[nPorts](
      ...
      annotation (Placement(transformation(extent={{90,-10},{70,10}})));
    AixLib.Fluid.Sensors.MassFlowRate senMasFlo[nPorts](redeclare package
        Medium = Medium) "Sensor that measures current mas flow rate"
