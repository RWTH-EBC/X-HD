...
equation
  ...
  if p < Fluid_out.p and Theta720 > 3.7*pi then
    ...
    0 = der(m)*u + m*der(u) - H_dot_in - H_dot_out - der(W_rev) - P_fric -
      Heat_port.Q_flow;
  ...
  elseif p > Fluid_out.p then
    ...
    //der(m) = -C_valve * Fluid_in.p * A_dis * sqrt(2*kappa/((kappa-1)*R_gas*Medium.temperature(state_in))
    //* abs(PI_p^(2/kappa)-PI_p^((2+kappa)/kappa)));
    der(m_out) = der(m);
    der(m_in) = 0;
  ...
  A_dis_cor = smooth(1, if der(m_out) < m_dot_out_avg then 5e-1*G_dot_out^(-1.3)
     else A_dis);
  ...
  PI_p = Fluid_out.p/Fluid_in.p;

  C_valve = 0.9;

  kappa = cp/cv;

  cv = Medium.specificHeatCapacityCv(state_dh);
