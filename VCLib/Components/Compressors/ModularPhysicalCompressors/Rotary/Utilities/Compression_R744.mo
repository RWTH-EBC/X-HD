...
model Compression_R744
  ...
  DynamicViscosity my(start=3e-5) "Dynamic viscosity of gas in chamber";

  Real Pr(start=1) "Prantl number of gas in chamber";

  Real Re(start=1e4) "Reynolds number of gas in chamber";
...
equation
  ...
  A_dis_cor = smooth(1, if der(m_out) < m_dot_out_avg then 2e0*G_dot_out^(-1.25)
     else A_dis);
  ...
  alpha_h = 0.037*lambda_t/L_ref*Pr^(1/3)*Re^(4/5);

  c1 = smooth(2, if der(V) > 0 then 6.18 else 2.28);

  Heat_port.T = T;

  Re = smooth(1, u_inf*L_ref*rho/my);
  //Re=1;
  my = Medium.dynamicViscosity(state_dh);
  //my = 3e-5;
  //lambda_t = Medium.thermalConductivity(state_dh);
  lambda_t = 0.1;
  //cp = Medium.specificHeatCapacityCp(state_dh);
  cp = 1;
  L_ref = 0.0001 + (2*pi - rem(Theta720, 2*pi))*(R + r)/2;
