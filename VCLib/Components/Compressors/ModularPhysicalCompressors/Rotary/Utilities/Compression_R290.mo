...
model Compression_R290
  ...
  MassFlowRate m_dot_out_avg;

  MassFlowRate m_dot_in_avg;
  ...
  Power P "mechanical Power needed";

  SpecificInternalEnergy u(start=550e3) "specific internal energy of gas";
  ...
  SpecificEnthalpy h(start=600e3) "Specific enthalpy of gas";
  ...
  Pressure p(start=3e5, max=700e5) "Pressure in compressor";
  ...
  SpecificHeatCapacity cp(max=3000) "Specific heat capacity for constant pressure";
...
equation
  ...
  alpha_h = smooth(1, if time > 0.05 then 0.037*lambda_t/L_ref_cor*Pr^(1/3)*Re^(4/5) else 20);
 ...
 // lambda_t = 1;
  ...
  //Pr = Medium.prandtlNumber(state_dh); // R290 gives non physical values
  //Pr = my*cp/lambda_t;
  //Pr = 1;
