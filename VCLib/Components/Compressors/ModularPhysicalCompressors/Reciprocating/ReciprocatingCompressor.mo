...
model ReciprocatingCompressor
  ...
  parameter
    VCLib.Components.Compressors.ModularCompressors.CompressorChoices.Efficiency_based_models.RotaryCompressors.BaseClasses.mediumtypes
    mediumtype;
  parameter Modelica.Units.SI.Temperature Tevap;
  parameter Modelica.Units.SI.Pressure Pevap;
  parameter Modelica.Units.SI.Temperature Tcond;
  parameter Modelica.Units.SI.Pressure Pcond;
//   parameter Integer nportsevap;
//   parameter Integer nportscond;
  parameter Modelica.Units.SI.AngularVelocity speed;

  BaseClasses.Utilities.Geometry.Volumes volumes annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=-90,
        origin={-10,68})));
  ...
  Modelica.Fluid.Sources.FixedBoundary Evaporator_out(
    ...
    redeclare package Medium =
        VCLib.Components.Compressors.ModularCompressors.CompressorChoices.Efficiency_based_models.RotaryCompressors.BaseClasses.Medium,
    nPorts=1,
    T(displayUnit="K") = Tevap,
    p(displayUnit="bar") = Pevap)
  ...
  Modelica.Fluid.Sources.FixedBoundary Condenser_in(
    ...
    redeclare package Medium =
        VCLib.Components.Compressors.ModularCompressors.CompressorChoices.Efficiency_based_models.RotaryCompressors.BaseClasses.Medium,
    T(displayUnit="degC") = Tcond,
    nPorts=1,
    p=Pcond) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=180,
        origin={88,40})));
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor
    ...
    "Thermal conduction between cylinder and ambient" annotation (Placement(
        ...
        rotation=90,
        origin={-12,-36})));
  final parameter Modelica.Units.SI.ThermalConductance G=BaseClasses.Utilities.Records.Geometry_Roskoch.G_wall_env
    "Constant thermal conductance of material";
  BaseClasses.Utilities.ThermalConductor_Gas_Cylinder
    thermalConductor_Gas_Cylinder annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-12,12})));
  BaseClasses.Utilities.closedVolume2 Piston
    annotation (Placement(transformation(extent={{-20,30},{0,50}})));
equation
//    (Tevap,Pevap,nportsevap,Tcond,Pcond,nportscond,speed)=BaseClasses.Utilities.ReciprocatingMediumSelect(mediumtypes=mediumtype);
