within VCLib.Components.Compressors.Validation;
model ModularInactiveCompressors
  ...
  replaceable ModularCompressors.ModularCompressors modCom(
    ...
    redeclare model IsentropicEfficiency =
      Utilities.IsentropicEfficiency.SpecifiedEfficiencies.Buck_R134aR450aR1234yfR1234zee_VarDisVol_Rotary,
    useExt=true,
    show_parCom=false,
    show_parCon=false,
  ...
  AixLib.Fluid.Actuators.Valves.SimpleValve simVal(
    redeclare package Medium = Medium,
    m_flow_start=0.025,
    m_flow_small=1e-6,
    Kvs=1.4) "Model of a simple valve to simulate pressure losses" annotation (
      Placement(transformation(
        extent={{-10,10},{10,-10}},
        rotation=-90,
        origin={40,-40})));
  AixLib.Fluid.Sources.FixedBoundary sink(
    ...
    nPorts=1,
    p=pInl,
    T=TInl) "Sink with constant pressure and temperature" annotation (Placement(
        transformation(
        extent={{10,-10},{-10,10}},
        rotation=-90,
        origin={40,-72})));

  AixLib.Controls.Interfaces.ModularHeatPumpControlBus dataBus(nCom=nCom)
