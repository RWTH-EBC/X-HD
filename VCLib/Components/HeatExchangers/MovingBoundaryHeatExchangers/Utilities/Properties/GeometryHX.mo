within VCLib.Components.HeatExchangers.MovingBoundaryHeatExchangers.Utilities.Properties;
record GeometryHX
  ...
  parameter Integer nFloCha = 25
    ...
    annotation(Dialog(tab="General", group="General"));
  parameter Modelica.Units.SI.Length lFloCha=1.5
    "Total length of each flow channel"
    annotation (Dialog(tab="General", group="General"));
  parameter Modelica.Units.SI.Length sFloCha=0.002
    "Thickness of each flow channel's wall"
    annotation (Dialog(tab="General", group="General"));

  parameter Modelica.Units.SI.Diameter dFloChaCir=0.01
    "Diameter of each flow channel of a circular heat exchanger" annotation (
      Dialog(
      tab="General",
      group="Circular heat exchanger",
      enable=(CroSecGeo == Types.GeometryCV.Circular)));

  parameter Modelica.Units.SI.Area ACroSecPla=7.85e-5
    "Cross-sectional area of each flow channel of a plate heat exchanger"
    annotation (Dialog(
      tab="General",
      group="Plate heat exchanger",
      enable=(CroSecGeo == Types.GeometryCV.Plate)));
  parameter Modelica.Units.SI.Length uFloChaPla=0.0315
    "Perimeter of each flow channel of a plate heat exchanger" annotation (
      Dialog(
      tab="General",
      group="Plate heat exchanger",
      enable=(CroSecGeo == Types.GeometryCV.Plate)));
  ...
  //
  final parameter Modelica.Units.SI.Length l=nFloCha*lFloCha
    "Length of all flow channels";
  final parameter Modelica.Units.SI.Diameter dFloCha=if (CroSecGeo == Types.GeometryCV.Circular)
       then dFloChaCir else (4*ACroSecPla)/uFloChaPla
    "Hydraulic diameter of each flow channel";

  final parameter Modelica.Units.SI.Area ACroSec=if (CroSecGeo == Types.GeometryCV.Circular)
       then nFloCha*Modelica.Constants.pi*dFloChaCir^2/4 else nFloCha*
      ACroSecPla "Cross-sectional area of all flow channels";
  final parameter Modelica.Units.SI.Area ACroSecFloCha=ACroSec/nFloCha
    "Cross-sectional area of each flow channel";

  final parameter Modelica.Units.SI.Area ACroSecWal=nFloCha*Modelica.Constants.pi
      /4*((dFloCha + sFloCha)^2 - dFloCha^2)
    "Cross-sectional area of the wall of all flow channels";
  final parameter Modelica.Units.SI.Area ACroSecWalFloCha=ACroSecWal/nFloCha
    "Cross-sectional area of the wall of each flow channel";

  final parameter Modelica.Units.SI.Area AHeaTra=if (CroSecGeo == Types.GeometryCV.Circular)
       then nFloCha*Modelica.Constants.pi*dFloChaCir*lFloCha else nFloCha*
      uFloChaPla*lFloCha "Heat transfer area of each flow channel";
  final parameter Modelica.Units.SI.Area AHeaTraFloCha=AHeaTra/nFloCha
    "Heat transfer area of each flow channel";

  final parameter Modelica.Units.SI.Volume VCroSec=lFloCha*ACroSec
    "Volume of all flow channels";
  final parameter Modelica.Units.SI.Volume VCroSecFloCha=VCroSec/nFloCha
    "Volume of each flow channel";

  final parameter Modelica.Units.SI.Volume VCroSecWal=lFloCha*ACroSecWal
    "Volume of the wall of all flow channels";
  final parameter Modelica.Units.SI.Volume VCroSecWalFloCha=VCroSecWal/nFloCha
