within VCLib.Components.HeatExchangers.MovingBoundaryHeatExchangers.Validation.HeatExchangers;
model BaseExampleCondenser
  ...
  parameter Modelica.Units.SI.MassFlowRate m_flow_sec=0.5
    "Nominal mass flow rate"
  ...
  AixLib.Fluid.Sources.Boundary_ph sinPri(
    ...
    use_p_in=true,
    p=pOut,
    nPorts=1) "Sink that provides a constant pressure" annotation (
  ...
  SimpleHeatExchangers.SimpleCondenser eva(
    ...
    useFixModCV=true,
    modCVPar=Utilities.Types.ModeCV.SH)
  ...
  AixLib.Fluid.Sources.Boundary_ph sinSec(
    ...
    nPorts=1,
    redeclare package Medium = MediumSec,
    p=101325) "Sink that provides a constant pressure" annotation (
      Placement(transformation(
        extent={{10,10},{-10,-10}},
        rotation=180,
        origin={-50,30})));
