within VCLib.Components.HeatExchangers.MovingBoundaryHeatExchangers.Examples;
partial model BaseExample
  ...
  //
  parameter Modelica.Units.SI.Temperature TOut=263.15
  ...
  parameter Modelica.Units.SI.MassFlowRate m_flow_sec=3.5
    "Nominal mass flow rate"
    annotation (Dialog(tab="General", group="Secondary fluid"));
  parameter Modelica.Units.SI.Temperature TInl=293.15
    "Actual temperature at outlet conditions"
    annotation (Dialog(tab="General", group="Secondary fluid"));
  ...
  //
  AixLib.Fluid.Sources.MassFlowSource_h souPri(
  ...
  AixLib.Fluid.Sources.Boundary_ph sinPri(
    redeclare package Medium = Medium,
    use_p_in=true,
    p=pOut,
  ...
  ModHeaExcMod modHeaExc(
    ...
    typHX=Utilities.Types.TypeHX.CounterCurrent,
    nHeaExc=1)
  ...
  AixLib.Fluid.Sources.Boundary_ph sinSec(
    use_p_in=false,
    redeclare package Medium = MediumSec,
    p=101325,
    nPorts=1) "Sink that provides a constant pressure" annotation (Placement(
  ...
  Components.Interfaces.PortAThroughPortsB portAThroughPortsB(nVal=modHeaExc.nHeaExc,
      redeclare package Medium = MediumSec) annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=-90,
        origin={10,12})));
  Components.Interfaces.PortsAThroughPortB portsAThroughPortB(nVal=modHeaExc.nHeaExc,
      redeclare package Medium = MediumSec) annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
