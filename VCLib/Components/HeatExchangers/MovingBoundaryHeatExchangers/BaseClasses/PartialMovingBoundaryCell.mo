...
partial model PartialMovingBoundaryCell
  ...
  parameter Modelica.Units.SI.CoefficientOfHeatTransfer AlpSC=2000 "Effective coefficient of heat transfer between the wall and fluid of the
    supercooled regime" annotation (Dialog(
      tab="Heat transfer",
      group="Heat transfer coefficient",
      enable=not useHeaCoeMod));
  parameter Modelica.Units.SI.CoefficientOfHeatTransfer AlpTP=7500 "Effective coefficient of heat transfer between the wall and fluid of the
    two-phase regime" annotation (Dialog(
      tab="Heat transfer",
      group="Heat transfer coefficient",
      enable=not useHeaCoeMod));
  parameter Modelica.Units.SI.CoefficientOfHeatTransfer AlpSH=2500 "Effective coefficient of heat transfer between the wall and fluid of the
  ...
  parameter Modelica.Units.SI.AbsolutePressure pIni=2e5
    "Start value of absolute pressure"
    annotation (Dialog(tab="Advanced", group="Initialisation"));
  parameter Modelica.Units.SI.SpecificEnthalpy dhIni=10 "Difference between inlet and outlet enthalpies 
...
public
  ...
  //
public
  Modelica.Units.SI.AbsolutePressure p(stateSelect=StateSelect.prefer)
    "Pressure of the working fluid (assumed to be constant)";

  Modelica.Units.SI.Temperature TInl=Medium.temperature_ph(p=p, h=hInl)
    "Temperature at the inlet of design direction";
  Modelica.Units.SI.Temperature TSC=Medium.temperature(state=SC)
    "Temperature of the supercooled regime";
  Modelica.Units.SI.Temperature TSCTP=Medium.temperature_ph(p=p, h=hSCTP) "Temperature at the boundary between the supercooled and two-phase 
    regime";
  Modelica.Units.SI.Temperature TTP=Medium.temperature(state=TP)
    "Temperature of the two-phase regime";
  Modelica.Units.SI.Temperature TTPSH=Medium.temperature_ph(p=p, h=hTPSH) "Temperature at the boundary between the two-phase and superheated 
    regime";
  Modelica.Units.SI.Temperature TSH=Medium.temperature(state=SH)
    "Temperature of the superheated regime";
  Modelica.Units.SI.Temperature TOut=Medium.temperature_ph(p=p, h=hOut)
    "Temperature at the outlet of design direction";

  Modelica.Units.SI.Density dInlDes=Medium.density_ph(p=p, h=hInl)
    "Density at the inlet of design direction";
  Modelica.Units.SI.Density dSC=Medium.density(state=SC)
    "Density of the supercooled regime";
  Modelica.Units.SI.Density dSCTP=Medium.density_ph(p=p, h=hSCTP) "Density at the boundary between the supercooled and two-phase 
    regime";
  Modelica.Units.SI.Density dTP=Medium.density(state=TP)
    "Density of the two-phase regime";
  Modelica.Units.SI.Density dTPSH=Medium.density_ph(p=p, h=hTPSH) "Density at the boundary between the two-phase and superheated 
    regime";
  Modelica.Units.SI.Density dSH=Medium.density(state=SH)
    "Density of the superheated regime";
  Modelica.Units.SI.Density dOutDes=Medium.density_ph(p=p, h=hOut)
  ...
  Modelica.Units.SI.SpecificEnthalpy hSC
    "Specific enthalpy of the supercooled regime";
  Modelica.Units.SI.SpecificEnthalpy hSCTP(stateSelect=StateSelect.prefer) "Specific enthalpy at the boundary between the supercooled and two-phase 
    regime";
  Modelica.Units.SI.SpecificEnthalpy hTP
    "Specific enthalpy of the two-phase regime";
  Modelica.Units.SI.SpecificEnthalpy hTPSH(stateSelect=StateSelect.prefer) "Specific enthalpy at the boundary between the two-phase and superheated 
    regime";
  Modelica.Units.SI.SpecificEnthalpy hSH
    "Specific enthalpy of the superheated regime";
  Modelica.Units.SI.SpecificEnthalpy hOut(stateSelect=StateSelect.prefer)
    "Specific enthalpy at the outlet of design direction";

  Modelica.Units.SI.Density dLiq=Medium.bubbleDensity(sat=TPSat)
    "Density at bubble line";
  Modelica.Units.SI.Density dVap=Medium.dewDensity(sat=TPSat)
    "Density at dew line";
  Modelica.Units.SI.SpecificEnthalpy hLiq=Medium.bubbleEnthalpy(sat=TPSat)
    "Specific enthalpy at bubble line";
  Modelica.Units.SI.SpecificEnthalpy hVap=Medium.dewEnthalpy(sat=TPSat)
...
public
  ...
  //
public
  Modelica.Units.SI.MassFlowRate m_flow_inl(start=m_flow_startInl, fixed=
        useFixStaVal) "Mass flow rate flowing into the system";
  Modelica.Units.SI.MassFlowRate m_flow_SCTP(start=m_flow_startSCTP, fixed=
        useFixStaVal) "Mass flow rate flowing out of the supercooled regime and into the two-phase
    regime";
  Modelica.Units.SI.MassFlowRate m_flow_TPSH(start=m_flow_startTPSH, fixed=
        useFixStaVal) "Mass flow rate flowing out of the two-phase regime and into the superheated
    regime";
  Modelica.Units.SI.MassFlowRate m_flow_out(start=m_flow_startOut, fixed=
        useFixStaVal) "Mass flow rate flowing out of the system";
  ...
  //
public
  Modelica.Units.SI.SpecificHeatCapacity cpSC=Medium.specificHeatCapacityCp(SC)
    "Density of the supercooled regime";
  Modelica.Units.SI.SpecificHeatCapacity cpSH=Medium.specificHeatCapacityCp(SH)
    "Density of the superheated regime";

  Modelica.Units.SI.ThermalConductance kASC
    "Effective thermal conductance of th supercooled regime";
  Modelica.Units.SI.ThermalConductance kATP
    "Effective thermal conductance of th two-phase regime";
  Modelica.Units.SI.ThermalConductance kASH
    "Effective thermal conductance of th superheated regime";

  Modelica.Units.SI.TemperatureDifference dTSC
    "Temperature difference between the wall and the supercooled regime";
  Modelica.Units.SI.TemperatureDifference dTTP
    "Temperature difference between the wall and the two-phase regime";
  Modelica.Units.SI.TemperatureDifference dTSH
    "Temperature difference between the wall and the superheated regime";

protected
  Modelica.Units.SI.HeatFlowRate Q_flow_SC
    "Heat flow rate from between the wall and the supercooled regime";
  Modelica.Units.SI.HeatFlowRate Q_flow_TP
    "Heat flow rate from between the wall and the two-pahse regime";
  Modelica.Units.SI.HeatFlowRate Q_flow_SH
