...
model HeatPumpandStorage_prototype
  ...
  Controls.Control_Pump control_Pump(dp_max_HP=50000, dp_max_demand=building.dp_max_building)
    annotation (Placement(transformation(extent={{40,56},{60,76}})));
  Buildings.Building building
    annotation (Placement(transformation(extent={{24,6},{44,26}})));
  HeatPumpandStorages.HeatPump_Carnot heatPump_Carnot annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-80,-6})));
equation
  ...
  connect(pumpHP.port_b, heatPump_Carnot.port_a1) annotation (Line(points={{-46,
          -36},{-50,-36},{-50,-16},{-70,-16}}, color={0,127,255}));
  connect(heatPump_Carnot.port_b1, HeatingCoil.port_a) annotation (Line(points=
          {{-70,3.8},{-52,3.8},{-52,34},{-44,34}}, color={0,127,255}));
  connect(heatPump_Carnot.Q_flow_HP_out, control.Q_HP_actual1) annotation (Line(
        points={{-90,4.2},{-54,4.2},{-54,76},{-32,76}}, color={0,0,127}));
  connect(control.switch_HeatPump_OnOff, heatPump_Carnot.HP_OnOff) annotation (
      Line(points={{-11.2,70},{-106,70},{-106,-16.8},{-80,-16.8}}, color={0,0,
          127}));
  connect(temperature.port, heatPump_Carnot.port_b1) annotation (Line(points={{
          -76,24},{-74,24},{-74,3.8},{-70,3.8}}, color={0,127,255}));
  connect(heatPump_Carnot.Q_flow_HP_out, greaterThreshold.u) annotation (Line(
        points={{-90,4.2},{-62,4.2},{-62,-63},{-33,-63}}, color={0,0,127}));
  connect(heatPump_Carnot.Q_flow_HP_out, control_Pump.Q_HP_actual) annotation (
      Line(points={{-90,4.2},{-26,4.2},{-26,72.2},{39.4,72.2}}, color={0,0,127}));
